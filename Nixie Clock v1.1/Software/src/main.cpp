...
bool minuteButtonPressed = false;  // True if button physical press detected
unsigned long timeMinuteButtonPressed = 0;  // Used for debouncing and short/long press handling

bool hourButtonPressed = false;  // True if button physical press detected
unsigned long timeHourButtonPressed = 0;  // Used for debouncing and short/long press handling
...
void buttonsProcess() {
    ...
    // Minute Button handling and software debouncing
    if (digitalRead(minuteButtonPin) == HIGH)
    {
        if(minuteButtonPressed == false)
        {
            // Button pressed detected for the first time
        ...
        if((unsigned long) (millis() - timeMinuteButtonPressed) >= longPressTime) {
        ...
        if(minuteButtonPressed && (unsigned long) (millis() - timeMinuteButtonPressed) >= debounceDelay)
        {
            shortMinuteButtonAction();
    ...
    // Hour Button handling and software debouncing
    if (digitalRead(hourButtonPin) == HIGH)
    {
        if(hourButtonPressed == false)
        {
            // Button pressed detected for the first time
        ...
        if((unsigned long) (millis() - timeHourButtonPressed) >= longPressTime) {
            // continuous long press action
            shortHourButtonAction();
            delay(100);
        }
    } else {
        if(hourButtonPressed && (unsigned long) (millis() - timeHourButtonPressed) >= debounceDelay)
        {
            shortHourButtonAction();
